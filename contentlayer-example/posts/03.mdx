...
```js
// 跳转时
this.navigateTo(this.pages.EmailVerificationPage, { fatherPage: this });

// 接收方
const fatherPageContext = global.routeParams[this.props.route.params.pageUUID];
```

## 时间戳转换

> 时间戳为 10 位需\*1000，时间戳为 13 位的话不需乘 1000
...
```js
const date = new Date('2022-01-20 10:39:49:123');
// 有三种方式获取
...
```shell
// cart delete
onItemDelete = (item: DOrder) => {
    console.log('item:', item);
    const dataList = this._rspCartListPage.cartOrderList.map(list => {
        list.order_list = list.order_list.filter(s => s.id != item.id);
        return list;
    });
